...
!!! THE MAIN ACTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  allocate(tmp_I(params%geom%n1b,params%geom%n2b,params%geom%n3b    ))        
  allocate(  Ident(params%geom%n1b,params%geom%n2b,params%geom%n3b    )) 

  I1 = 0.0_rk
  tmp_I = 0.0_rk
  Ident = 0.0_rk
  call Dx1(tmp_I,   X_metric(:,:,:,1,1),params%geom%dx1 )
  Ident = Ident + tmp_I 
  call Dx2(tmp_I,   X_metric(:,:,:,2,1),params%geom%dx2 )
  Ident = Ident + tmp_I 
  call Dx3(tmp_I,   X_metric(:,:,:,3,1),params%geom%dx3 )
  Ident = Ident + tmp_I 
  I1 = I1 + sum(ABS(Ident)) 
  Ident = Ident*0
  ...
  tmp_I = 0.0_rk
  Ident = 0.0_rk
  call Dx1(tmp_I,   X_metric(:,:,:,1,2),params%geom%dx1 )
  Ident = Ident + tmp_I  
  call Dx2(tmp_I,   X_metric(:,:,:,2,2),params%geom%dx2 )
  Ident = Ident + tmp_I 
  call Dx3(tmp_I,   X_metric(:,:,:,3,2),params%geom%dx3 )
  Ident = Ident + tmp_I  
  I2 = I2 + sum(ABS(Ident)) 
  Ident = Ident*0 
  ...
  tmp_I = 0.0_rk
  Ident = 0.0_rk
  call Dx1(tmp_I,   X_metric(:,:,:,1,3),params%geom%dx1 )
  Ident = Ident + tmp_I  
  call Dx2(tmp_I,   X_metric(:,:,:,2,3),params%geom%dx2 )
  Ident = Ident + tmp_I 
  call Dx3(tmp_I,   X_metric(:,:,:,3,3),params%geom%dx3 )
  Ident = Ident + tmp_I 
  I3 = I3 + sum(ABS(Ident)) 
  Ident = Ident*0
  write(*,*) '############ Unit Test Metric sinusiodial 3D grid ############'
  write(*,*) 'The 1. identity is:', I1
  write(*,*) 'The 2. identity is:', I2
  write(*,*) 'The 3. identity is:', I3
  write(*,*) 'The total identity is:', I1+I2+I3
  
  if (I1+I2+I3  < 5.0E-008) then
        write(*,*) 'Unit Test Metric PASSED'
  else
        write(*,*) 'Unit Test Metric FAILED'
        stop
  end if    
  ...
  ! clean
  deallocate(q1)
  ! compare current simualtion with reference data   
