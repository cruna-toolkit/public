module diff_pen_core_o6_i_c_p_omp
  ...
  character(len=max_length_parameter), parameter, public :: diff_name = 'diff_pen_core_o6_i_c_p_omp'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    real(kind=rk), dimension(:,:), allocatable, save   :: q_1            ! single fries
    ...
    integer                      , save                :: tnr            ! number of threads
    integer                                            :: i
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (.not.allocated(dl)) then
       ...
       ! number of threads
       call get_parameter(tnr,'parallelism.tnr_omp',default = 2)
       !$ call omp_set_num_threads( tnr )
       ! allocate single fries
       allocate(q_1(size(q,1),1))
    end if

    !$omp parallel num_threads(tnr) shared(q) private(q_1,i) firstprivate(dx,stencil,dl,dc,du,du2,fbrat,fbnorm,ipiv) 
    !$omp do schedule(dynamic)
    do i = 1,size(q,2)
       q_1(:,1) = q(:,i)
       call diff_main(q_1,dx,stencil,dl,dc,du,du2,fbrat,fbnorm,ipiv)
       q(:,i) = q_1(:,1)
    end do
    !$omp end do
    !$omp end parallel
...
end module diff_pen_core_o6_i_c_p_omp
