module filter_pen_core_o6_i_c_o
  ...
  character(len=max_length_parameter), parameter, public :: filter_name = 'filter_pen_core_o6_i_c_o'

  real(kind=rk), dimension(4)        , save              :: stencil      ! right explicit stencil A
  real(kind=rk), dimension(2,7)      , save              :: coefficients_for_boundary
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do j = 1,n2
       do i = 4,(n1-3)
          q_tmp(i,j) = a0*q(i,j) + a1*(q(i+1,j) + q(i-1,j)) + a2*(q(i+2,j) + q(i-2,j)) + a3*(q(i+3,j) + q(i-3,j)) ! the factor 1/2 is already considered in stencil
       end do
    end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    real(kind=rk) , dimension(2,7)  , intent(out)       :: coefficients_for_boundary
    real(kind=rk) , dimension(4)    , intent(out)       :: stencil
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    a0       = 11.0_rk/16.0_rk +  5.0_rk*alpha/ 8.0_rk

    a1       = 15.0_rk/32.0_rk + 17.0_rk*alpha/16.0_rk
    a1       = a1/2.0_rk

    a2       = -3.0_rk/16.0_rk +  3.0_rk*alpha/ 8.0_rk
    a2       = a2/2.0_rk

    a3       =  1.0_rk/32.0_rk -  1.0_rk*alpha/16.0_rk
    a3       = a3/2.0_rk

    stencil  = (/ a0, a1, a2, a3 /)

    ! point 2 (non-per case)
    coefficients_for_boundary(1,1) =  1.0_rk/64.0_rk + 31.0_rk*alpha/32.0_rk
    coefficients_for_boundary(1,2) = 29.0_rk/32.0_rk +  3.0_rk*alpha/16.0_rk
    coefficients_for_boundary(1,3) = 15.0_rk/64.0_rk + 17.0_rk*alpha/32.0_rk
    coefficients_for_boundary(1,4) = -5.0_rk/16.0_rk +  5.0_rk*alpha/ 8.0_rk
    coefficients_for_boundary(1,5) = 15.0_rk/64.0_rk - 15.0_rk*alpha/32.0_rk
    coefficients_for_boundary(1,6) = -3.0_rk/32.0_rk +  3.0_rk*alpha/16.0_rk
    coefficients_for_boundary(1,7) =  1.0_rk/64.0_rk -  1.0_rk*alpha/32.0_rk

    ! point 3 (non-per case)
    coefficients_for_boundary(2,1) = -1.0_rk/64.0_rk +  1.0_rk*alpha/32.0_rk
    coefficients_for_boundary(2,2) =  3.0_rk/32.0_rk + 13.0_rk*alpha/16.0_rk
    coefficients_for_boundary(2,3) = 49.0_rk/64.0_rk + 15.0_rk*alpha/32.0_rk
    coefficients_for_boundary(2,4) =  5.0_rk/16.0_rk +  3.0_rk*alpha/ 8.0_rk
    coefficients_for_boundary(2,5) =-15.0_rk/64.0_rk + 15.0_rk*alpha/32.0_rk
    coefficients_for_boundary(2,6) =  3.0_rk/32.0_rk -  3.0_rk*alpha/16.0_rk
    coefficients_for_boundary(2,7) = -1.0_rk/64.0_rk +  1.0_rk*alpha/32.0_rk
...
end module filter_pen_core_o6_i_c_o
